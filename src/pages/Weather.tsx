...
export default function WeatherPage() {
  ...
  return <div className="min-h-screen overflow-x-hidden relative">
      {premiumSettings.animatedBackgrounds && <AnimatedWeatherBackground condition={weatherData?.mostAccurate?.currentWeather?.condition} sunrise={weatherData?.mostAccurate?.currentWeather?.sunrise} sunset={weatherData?.mostAccurate?.currentWeather?.sunset} moonPhase={weatherData?.mostAccurate?.currentWeather?.moonPhase} />}
        ...
        <Card className="mb-6 relative z-[1000] overflow-hidden rounded-2xl border-white/20 shadow-xl">
          ...
          <div className="bg-gradient-to-r from-sky-500/70 via-blue-600/60 to-indigo-700/70 p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                ...
                {user && selectedLocation && weatherData && <OfflineDownloadButton location={selectedLocation.name} latitude={selectedLocation.lat} longitude={selectedLocation.lon} currentWeather={weatherData.mostAccurate?.currentWeather} hourlyForecast={weatherData.mostAccurate?.hourlyForecast || []} dailyForecast={weatherData.mostAccurate?.dailyForecast || []} />}
          ...
          <CardContent className="p-4 sm:p-6 bg-background/50 backdrop-blur-md space-y-4">
            ...
            {selectedLocation && <div className="pt-3 border-t border-border/20">
                {user ? <PredictionDialog location={selectedLocation?.name || "Unknown"} latitude={selectedLocation?.lat || 0} longitude={selectedLocation?.lon || 0} isImperial={isImperial} onPredictionMade={() => {}} /> : <LockedFeature isLocked={true}>
